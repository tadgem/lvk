...
namespace lvk {
  ...
  void                                CreateUniformBuffers(VkState& vk, Vector<VkBuffer>& uniformBuffersFrames, Vector<VmaAllocation>& uniformBuffersMemoryFrames, Vector<void*>& uniformBufferMappedMemoryFrames)
  {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
      CreateBufferVMA(vk, bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, uniformBuffersFrames[i], uniformBuffersMemoryFrames[i]);

      VK_CHECK(vmaMapMemory(vk.m_Allocator, uniformBuffersMemoryFrames[i], &uniformBufferMappedMemoryFrames[i]))
    }
  ...
  template<typename _Ty>
  void  CreateVertexBuffer(VkState& vk, Vector<_Ty> verts, VkBuffer& buffer, VmaAllocation& deviceMemory)
  {
